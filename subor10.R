...
subset1=subset[subset$Intonation=="Declarative",]
subset2=subset[subset$Intonation=="Interrogative",]
...
plot1=ggplot(subset1, aes(x=Constituent, y=fourthpitch)) + stat_summary(fun.y=mean, geom="point") + stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) + facet_grid(Focus ~ Structure, scales = "fixed") + labs(title="Declarative") + ylab("Duration")

plot2=ggplot(subset2, aes(x=Constituent, y=fourthpitch)) + stat_summary(fun.y=mean, geom="point") + 
stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) + 
facet_grid(Focus ~ Structure, scales = "fixed") + labs(title="Interrogative") + ylab("Duration")
...
ggplot(brack.wide, aes(x=Structure, y=fourthpitch.11)) + stat_summary(fun.y=mean, geom="point") + 
  #stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + 
  stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) #+ facet_grid(Focus ~ Structure, scales = "fixed") 


ggplot(brack.wide, aes(x=Structure, y=fourthpitch.11)) + geom_boxplot() + facet_grid(~Intonation)#+ facet_grid(Focus ~ Structure, scales = "fixed") 


# Model: Does pitch cue phrasing?
pitchModel=glm(Structure~scale(fourthpitch.11)*Intonation+scale(fourthpitch.11)*Focus,family="binomial", data=brack.wide) 
summary(pitchModel)

# what about just in interrogatives?
subsInterrogative=subset[brack.wide$Intonation=="Interrogative",]
pitchModelInter=glm(Structure~scale(fourthpitch.11),family="binomial", data=subsInterrogative) 
