...
facet_grid(Focus ~ Structure, scales = "fixed") + opts(title="Declarative")  + ylab("LatePitch")

plot2=ggplot(subset2, aes(x=word, y=fourthpitch)) + stat_summary(fun.y=mean, geom="point") + 
stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) + 
facet_grid(Focus ~ Structure, scales = "fixed") + opts(title="Interrogative")  + ylab("LatePitch")
...
stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) + 
facet_grid(Focus ~ bracketing, scales = "fixed") + opts(title="Declarative") + ylab("Maxpitch")

plot2=ggplot(subset2, aes(x=Constituent, y=maxpitch)) + stat_summary(fun.y=mean, geom="point") + 
stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) + 
facet_grid(Focus ~ bracketing, scales = "fixed") + opts(title="Interrogative") + ylab("MaxPitch")
...
plot1=ggplot(subset1, aes(x=Constituent, y=duration)) + stat_summary(fun.y=mean, geom="point") + stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) + facet_grid(Focus ~ Intonation, scales = "fixed") + opts(title="(AB)C") + ylab("Duration")

plot2=ggplot(subset2, aes(x=Constituent, y=duration)) + stat_summary(fun.y=mean, geom="point") + 
stat_summary(fun.y = "mean", geom="line", aes(group=Focus)) + stat_summary(fun.data = "mean_cl_boot", geom="errorbar",size=0.6, width=.15,) + 
facet_grid(Focus ~ Intonation, scales = "fixed") + opts(title="A(BC)") + ylab("Duration")
